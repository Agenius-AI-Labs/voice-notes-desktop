...
**Severity:** High
**Status:** **Patched 2026-05-13.** OS keyring (macOS Keychain / Windows
Credential Manager / Linux Secret Service) is now the primary store via
`core/keystore.py`. SQLite is a transparent fallback for environments
without a keyring backend. One-shot migration on app startup moves
existing DB-stored keys into the keyring.
...
**Severity:** Medium
**Status:** **Patched 2026-05-13.** `requirements.lock.txt` generated by
`pip-compile`. CI enforces strict `pip-audit` against the lockfile plus a
"lockfile in sync with pyproject" check.
...
- [x] H1 — keyring-based API key storage. Landed.
- [x] M3 — `pip-audit` (strict, on lockfile) in CI plus lockfile-in-sync check. Landed.
- [x] L1 — WAV temp moved to user-data dir. Landed.
- [x] L2 — `logging` module replaces stderr `print`. Landed.
